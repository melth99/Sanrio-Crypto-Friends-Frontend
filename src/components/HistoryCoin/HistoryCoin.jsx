...
export default function HistoryCoin({ fetchHistory, historyData, setHistoryData, SearchCurrencies, searchQuery, fetchList }) {
    const initialState = {
        ...
        date: "", // YYYY-MM-DD
        symbols: "" // Can have multiple symbols separated by commas - not possible with current routing
        // need crypto symbol to operate
    };
    if (searchQuery) {
    ...
    async function handelCoinSearch() {
        try {
            const coinList = await fetchList()
            setCoinSearch(coinList["crypto"]["name_full"])
            console.log('handler,',coinList["crypto"])
        } catch (err) {
